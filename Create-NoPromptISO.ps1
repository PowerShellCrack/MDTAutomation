...
# ISO Original path
$SourceISOFullPath = "$scriptDirectory\LiteTouchPE_x64.iso"

If(-not(Test-Path $SourceISOFullPath)){
    Write-Host ("LiteTouchPE_x64.iso does not exist in path: [{0}].`nRun script from Deploymentshare's boot folder after Deploymentshare has been updated." -f (Split-Path $SourceISOFullPath -Parent)) -ForegroundColor Red
    Exit
}

# New ISO path
# it will be placed in same directory as original one
$ISODirectory = Split-Path $SourceISOFullPath -Parent
$NewISOFullPath = Join-Path $ISODirectory -ChildPath 'LiteTouchPE_x64-NoPrompt.iso'
...
# Mount ISO
$mountISO = Mount-DiskImage $SourceISOFullPath -PassThru
...
Finally{
    ...
    # dismount the iso.
    Dismount-DiskImage -ImagePath "$SourceISOFullPath" | Out-null
